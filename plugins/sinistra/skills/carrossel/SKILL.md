...
- **Node e Playwright:** pra renderizar o HTML em imagem, instalados pelo `/instalar` em
...
2. **Copiar** `${CLAUDE_PLUGIN_ROOT}/templates/carrossel/render.js` pra mesma pasta do `carrossel.html`. Não
   ...
   Rodar carregando o ambiente que o `/instalar` deixou pronto. Ele resolve sozinho a
   diferença entre Windows e Mac, então o comando é o mesmo nos dois:

   ```bash
   . "$HOME/.sinistra/runtime/env.sh" && node render.js
   ```

   Nunca rodar `node render.js` solto: sem esse ambiente o Playwright não acha o Chromium e
   ...
   Se `~/.sinistra/runtime/env.sh` não existir, o motor de imagem não foi instalado.
   **Não tentar instalar aqui**, parar e falar:
...
- Render: sempre carregando `~/.sinistra/runtime/env.sh` antes. Nunca rodar `npm install` dentro da pasta do cliente
