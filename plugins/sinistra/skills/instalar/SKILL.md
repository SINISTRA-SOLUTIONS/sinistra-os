...
description: >
  ...
  "pode me perguntar", "configura aí", "/instalar", ou logo depois de o sistema ser instalado
  na pasta pela primeira vez.
---

# /instalar: a entrevista do negócio

É aqui que o sistema aprende quem é a empresa. É a primeira coisa que roda depois de o
sistema ser instalado na pasta.
...
### 2. A pasta de trabalho

A pasta chega **vazia**: quem a criou foi o próprio cliente, antes de abrir o app. Criar a
estrutura antes de começar a entrevista, sem comentar nada disso com ele:

```
_memoria/      identidade/    marketing/     saidas/    dados/    scripts/
```

E copiar dois arquivos pra raiz da pasta:

| De | Pra | Pra que serve |
|---|---|---|
| `${CLAUDE_PLUGIN_ROOT}/templates/cliente/CLAUDE.md` | `CLAUDE.md` | carrega o contexto do negócio no início de toda conversa. Sem ele o sistema esquece a empresa a cada sessão |
| `${CLAUDE_PLUGIN_ROOT}/CATALOGO.md` | `CATALOGO.md` | o que cada ferramenta faz, na língua dele. Fica na pasta pra ele poder abrir e ler quando quiser |

O `CATALOGO.md` é **cópia, e o original é o do plugin**. Quem mantém a cópia em dia é a
`/abrir`, sozinha. Nunca editar a cópia: a edição se perde na primeira vez que o sistema
atualizar.

### 3. O motor de imagem

É o que transforma o carrossel em imagem. Se já existir `~/.sinistra/runtime/env.sh`, está
pronto, seguir. Se não existir, instalar **em silêncio**, sem comentar nada com o cliente.

Detectar o sistema com `uname -s` (`Darwin` é Mac, `MINGW`/`MSYS`/`CYGWIN` é Windows) e rodar
o instalador correspondente:

```bash
# Mac
sh "${CLAUDE_PLUGIN_ROOT}/templates/runtime/instalar-runtime.sh"

# Windows
powershell -NoProfile -ExecutionPolicy Bypass -File "${CLAUDE_PLUGIN_ROOT}/templates/runtime/instalar-runtime.ps1"
```

Baixa ~110 MB de Node e ~271 MB de Chromium, na pasta do usuário, sem senha de administrador.
Demora alguns minutos.

**Nunca instalar Node por outro caminho** (Homebrew, winget, instalador oficial). Todos pedem
senha de administrador em algum momento, e a maioria dos clientes não é administrador da
própria máquina.

A última linha da saída diz o que aconteceu:

| Saída | O que fazer |
|---|---|
| `OK` | seguir, está pronto |
| `AVISO windows-10` | instalou mesmo assim, seguir normal. Só anotar que essa máquina roda fora do suporte declarado do Playwright, pra saber onde olhar se der problema de imagem depois |
| `MACOS-ANTIGO` | ver abaixo |
| qualquer outra | seguir a entrevista e avisar na Fase 4 |

**`MACOS-ANTIGO` é bloqueio de verdade**, não é conservadorismo: o Chromium não tem build pra
macOS 13 e o download falha com erro. Não insistir e não tentar contornar. Registrar e avisar
na Fase 4.

**Em nenhum caso travar a entrevista.** Sem o motor de imagem o sistema inteiro continua
funcionando, o carrossel só entrega o texto e a legenda em vez das imagens.

---

## Fase 1: O perfil, deduzido e não perguntado

O perfil decide qual gabarito de regras aplicar (`${CLAUDE_PLUGIN_ROOT}/templates/perfis/regras-<perfil>.md`), mas
...
- `${CLAUDE_PLUGIN_ROOT}/templates/memoria/empresa.md` → `_memoria/empresa.md`
- `${CLAUDE_PLUGIN_ROOT}/templates/memoria/preferencias.md` → `_memoria/preferencias.md`
- `${CLAUDE_PLUGIN_ROOT}/templates/memoria/estrategia.md` → `_memoria/estrategia.md`
- `${CLAUDE_PLUGIN_ROOT}/templates/identidade/design-guide.md` → `identidade/design-guide.md`

Copiar, nunca mover: os gabaritos precisam continuar em `${CLAUDE_PLUGIN_ROOT}/templates/` pra próxima instalação
...
### `_memoria/regras.md`
Pegar o gabarito do perfil escolhido na Fase 1 (`${CLAUDE_PLUGIN_ROOT}/templates/perfis/regras-<perfil>.md`), adaptar com o nome do negócio e a estrutura de pastas mencionada nas respostas, e gravar como `_memoria/regras.md`.
...
Se o motor de imagem não tiver instalado, acrescentar uma linha só.

Quando foi falha de instalação:

> "Um detalhe: a parte que transforma o post em imagem não terminou de instalar. Todo o resto
> funciona normal. Avisa a Sinistra, é rápido de resolver do lado deles."

Quando o sistema do computador é antigo demais (nunca dizer "antigo", nunca culpar a máquina
dele):

> "Um detalhe: esse computador ainda não tem a atualização do sistema que eu preciso pra
> montar as imagens. Eu escrevo o post e a legenda normalmente, e você monta a arte onde
> preferir. Se quiser destravar as imagens também, fala com a Sinistra."
...
- Tudo que sair da entrevista vai pra pasta do cliente (`_memoria/`, `identidade/`). **Nunca gravar nada dentro de `${CLAUDE_PLUGIN_ROOT}`**: aquilo é o sistema da Sinistra, é trocado inteiro a cada atualização, e o que for gravado lá se perde
...
- Nunca pedir pro usuário mexer em pasta, renomear nada ou fechar programa. A pasta já está aberta na frente dele, e quem monta o que vai dentro dela é essa skill
